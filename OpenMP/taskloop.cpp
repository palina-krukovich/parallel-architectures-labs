...
bool mandelbrot_kernel(complex<double> c, vector<int> &pixel)
{
    ...
    // Replace abs(z) which is calculated like sqrt(z.real()*z.real() + z.imag()*z.imag()) with just square of complex number
    // because sqrt operation is costly 
...
int main(int argc, char **argv)
{
    ...
    std::ofstream ofs("mandelbrot-taskloop.ppm", std::ofstream::out);
    ofs << "P3" << std::endl;
